 ...
 * @retval  None
 */
void GPIO_External_LED_Init(void)
{
 /* Enable clock for GPIOB*/
    RCC ->APB2ENR |= RCC_APB2ENR_IOPBEN;
 /* Configure PB.0 in output mode, max speed 2 MHz. */
    GPIOB ->CRL &= ~GPIO_CRL_MODE0;
    GPIOB ->CRL |=  GPIO_CRL_MODE0_1;
 /* Configure PB.0 as  general purpose output push-pull */
    GPIOB ->CRL  &= ~GPIO_CRL_CNF0;
 /* Led OFF */
    GPIOB ->CRL  &= ~GPIO_ODR_ODR0;
